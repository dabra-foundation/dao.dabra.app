...
const allowedFaviconPaths = ['/dabra/']
...
export function AppContents(props: Props) {
  ...
  const title = realmName ? `${realmName}` : 'Dabra'

  // Note: ?v==${Date.now()} is added to the url to force favicon refresh.
  // Without it browsers would cache the last used and won't change it for different dabra
  ...
  const faviconUrl = useMemo(() => {
    ...
    }/dabra/${resourcePath}/favicon.ico?v=${Date.now()}`
