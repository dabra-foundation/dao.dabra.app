import { AnchorProvider, BorshInstructionCoder, Idl } from '@coral-xyz/anchor'
...
function adaptIdlForCoral(idl: any): Idl {
  if (!idl) return idl
  return {
    ...idl,
    accounts: idl.accounts?.map((acc: any) => ({
      ...
      type: {
        ...acc.type,
        kind: acc.type?.kind === 'enum' ? 'struct' : acc.type?.kind
      }
    }))
  }
}
      ...
      getDataUI: async (connection: Connection, data: Uint8Array) => {
        try {
          ...
          const decodedInstructionData = new BorshInstructionCoder(
            adaptIdlForCoral(nftClient.program.idl)
