...
export const YouVoted = ({ quorum }: { quorum: 'electoral' | 'veto' }) => {
  ...
  const { result: voteRecords } = useAsync(
    async () => {
  ...
  const getDelegatorVoteForQuorum = () => {
    if (
      // yes/no vote
      (quorum === 'electoral' && !voteRecords?.account.vote?.veto) ||
      // veto vote
      (quorum === 'veto' && voteRecords?.account.vote?.veto)
    ) {
      return voteRecords?.account.vote
    }
    return undefined
  }
