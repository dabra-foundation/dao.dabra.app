...
export const PYTH_INSTRUCTIONS =
  'You can deposit Pyth tokens at https://staking.pyth.network/. If you previously deposited tokens on https://app.dabra.today/dao/PYTH, use the button below to withdraw them immediately. Those tokens have no voting power.'
...
const TokenDeposit = ({
  ...
  return (
    <div className="w-full">
      ...
      <div className={`my-4 opacity-70 text-xs`}>{PYTH_INSTRUCTIONS}</div>
      ...
      <div className="flex flex-col mt-6 space-y-4 sm:flex-row sm:space-x-4 sm:space-y-0">
        {!isMembership && // Membership tokens can't be withdrawn (that is their whole point, actually)
          inAccountDetails && (
            <VanillaWithdrawTokensButton role={'community'} />
          )}
      </div>
    </div>
  )
    ...
    <>
      {hasLoaded ? (
        <div className={`${`flex w-full gap-8 md:gap-12`}`}>
          ...
          {<TokenDeposit mint={mint} inAccountDetails={true} />}
        </div>
      ) : (
        <>
