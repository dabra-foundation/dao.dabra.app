...
export const useSavePlans = (
  ...
  async function deposit(
    ...
    const walletAta = await SplToken.getAssociatedTokenAddress(
      ...
      TOKEN_PROGRAM_ID,
      new PublicKey(reserve.cTokenMint),
      new PublicKey(dabraWalletAddress),
      true,
    );
    ...
    plans:
      reservesInfo.data?.map((reserve) => {
        ...
        return {
          ...
          deposit: (amount: number, dabraWalletAddress: string) =>
            deposit(reserve.address, amount, dabraWalletAddress),
        };
      }) ?? [],
    positions,
  };
