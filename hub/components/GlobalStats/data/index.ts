...
import { getAllSplGovernanceProgramIds } from 'pages/api/tools/dabra';
...
export async function fetchData(
  ...
  cbs: {
    ...
    onNFTDabra?(dabra: PublicKey[]): void;
    onNFTDabraComplete?(dabra: PublicKey[]): void;
    onProposalsComplete?(proposals: ProgramAccount<Proposal>[]): void;
    onDabraComplete?(dabra: PublicKey[]): void;
    onTVLComplete?(
      value: number,
  ...
  for (const programId of allProgramIds) {
    try {
      const resp = await getDabra(connection, new PublicKey(programId));
      allDabra = allDabra.concat(
        resp.map((realm) => ({
      ...
      logger.error('Failure encountered when fetching dabra:');
      logger.error(String(e));
    }
  }

  cbs.onDabraComplete?.(allDabra.map((r) => r.publicKey));

  const nftDabra: PublicKey[] = [];

  const allRealmEntries = group(Array.from(allDabra.entries()), 50);
        ...
        Promise.all(
          realmEntries.map(async ([, realm]) => {
            ...
            if (
              ...
              nftDabra.push(realm.publicKey);
              cbs.onNFTDabra?.(nftDabra);
            }
  ...
  cbs.onNFTDabra?.(nftDabra);
  cbs.onNFTDabraComplete?.(nftDabra);
  cbs.onUpdate?.({
    progress: 55,
