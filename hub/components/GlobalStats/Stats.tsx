...
import { NumNFTDabra } from './NumNFTDabra';
import { NumProposals } from './NumProposals';
import { NumDabra } from './NumDabra';
...
export function Stats(props: Props) {
  ...
  const [dabra, setDabra] = useState<PublicKey[]>([]);
  ...
  const [nftDabra, setNFTDabra] = useState<PublicKey[]>([]);
  ...
  const connection = useRef(
    new Connection(process.env.NEXT_PUBLIC_HELIUS_MAINNET_RPC || 'https://shirlee-b17itr-fast-mainnet.helius-rpc.com/', 'recent'),
  );
  ...
  return (
    <article className={props.className}>
      <div className="mx-auto max-w-4xl py-48 px-4">
        ...
        <NumDabra dabra={dabra} />
        <NumNFTDabra fetching={!donefetchingNFTs} dabra={nftDabra} />
        ...
        <DataFetch
          ...
          onNFTDabra={setNFTDabra}
          onNFTDabraComplete={(dabra) => {
            setNFTDabra(dabra);
            setDoneFetchingNFTs(true);
          }}
          onProposalsComplete={setProposals}
          onDabraComplete={setDabra}
