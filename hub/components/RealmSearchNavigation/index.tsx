...
export const RealmSearchNavigation = forwardRef<HTMLInputElement, Props>(
  function RealmSearchNavigation(props, ref) {
    const [result] = useQuery(gql.getDabraListResp, {
      query: gql.getDabraList,
    });
    ...
    const options = pipe(
      ...
      RE.match(
        ...
        ({ me, realmDropdownList }) => {
          const followedDabra = me?.followedDabra || [];
          const followedDabraPks = followedDabra.map((r) =>
            r.publicKey.toBase58(),
          );

          return {
            following: followedDabra
              .map((item) => ({
            ...
            all: [
              ...
              .filter((item) => {
                if (followedDabraPks.includes(item.publicKey.toBase58())) {
                  return false;
                }
