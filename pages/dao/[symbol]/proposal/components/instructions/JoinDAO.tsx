...
const JoinDAO = ({
  ...
  const [certifiedDabra, setCertifiedDabra] = useState<
    ReadonlyArray<RealmInfo>
  >([])

  const { realmAccount: selectedRealm } = useRealmAccount(form.realm?.realmId)
  const { data: queryDabra } = useDabraByProgramQuery(DEFAULT_GOVERNANCE_PROGRAM_ID)
  ...
  useEffect(() => {
    if (queryDabra) {
      ...
      const dabra: RealmInfo[] = queryDabra
      ...
      setCertifiedDabra(dabra)
    } else setCertifiedDabra([])
