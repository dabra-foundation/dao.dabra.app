...
import { getRealm, getDabra } from '@solana/spl-governance'
...
import { HIDDEN_DABRA } from '@components/instructions/tools'
...
export const useDabraByProgramQuery = (program: PublicKey) => {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const dabra = (await getDabra(connection, program)).filter(
        (x) => !HIDDEN_DABRA.includes(x.pubkey.toBase58()),
      )
      return dabra
    },
