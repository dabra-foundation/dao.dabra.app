  ...
  useEffect(() => {
    const handleSetClient = async () => {
      try {
        ...
        if (!client) {
          console.error('Failed to initialize Mango client')
          return
        }
        ...
        try {
          ...
          console.error('Failed to fetch Mango group:', groupError)
          setMangoClient(client) // Still set the client even if group fetch fails
          setMangoGroup(null)
        }
      } catch (error) {
        console.error('Error initializing Mango client:', error)
        setMangoClient(null)
        setMangoGroup(null)
      }
    }
